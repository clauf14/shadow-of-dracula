...
func _ready():
	if Global.saved_terrain_assets != null:
		terrain.assets = Global.saved_terrain_assets
	else:
		# Default assignment if no saved terrain
		terrain.assets = load("res://demo/data/assets.tres")

	# Runtime logic for loading player and UI data
	...
	# Editor-only logic for setting up Sky3D
